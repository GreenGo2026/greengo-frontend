...
export default function ProductPage() {
  ...
  const whatsappHref = "https://wa.me/212664397031?text=" + encodeURIComponent(
  ...
  return (
    <div className={font} dir={isRTL ? "rtl" : "ltr"} style={{ minHeight: "100vh", background: "#FAF7F2" }}>
          ...
          <div className="flex flex-col gap-5">
            ...
            {product.in_stock && (
              <div className="flex flex-col gap-3">
                ...
                {/* Hidden on mobile: the sticky Buy Box below covers this CTA.
                    The qty stepper above stays visible on all breakpoints. */}
      ...
      {/* ── Sticky Buy Box — mobile only ──────────────────────────────────
          bottom-16 (not bottom-0): MobileBottomNav (App.tsx) is already
          fixed bottom-0 h-16 z-50 on every page, so this sits just above it.
          Only rendered in-stock, matching the main panel's own convention
          (the "Quantity + Add to cart" block above is likewise gated on
          product.in_stock -- there's no existing "out of stock" CTA state
          on this page to mirror here). */}
      {product.in_stock && (
        <div className="lg:hidden fixed bottom-16 inset-x-0 z-40 bg-white border-t border-gray-100 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] px-4 py-3">
          <div className={`flex items-center gap-3 ${isRTL ? "flex-row-reverse" : ""}`}>
            <div className="flex-shrink-0">
              <p className="text-[10px] text-gray-400 leading-none mb-0.5">
                {l === "fr" ? "Prix" : l === "ar" ? "السعر" : "Price"}
              </p>
              <p className="font-black text-[#0c3228] text-lg leading-none font-latin">
                {activePrice.toFixed(2)} MAD
              </p>
            </div>
            <button onClick={handleAdd}
              className={`flex-1 py-3 rounded-xl font-bold text-sm transition-all active:scale-[0.98] ${added ? "bg-green-600 text-white" : "bg-[#2E8B57] hover:bg-[#1F6B40] text-white"}`}>
              {added
                ? (l === "fr" ? "✓ Ajouté !" : l === "ar" ? "✓ تمت الإضافة" : "✓ Added!")
                : qty > 0
                ? (l === "fr" ? "Ajouter encore" : l === "ar" ? "إضافة أخرى" : "Add more")
                : (l === "fr" ? "🛒 Ajouter au panier" : l === "ar" ? "🛒 أضف للسلة" : "🛒 Add to cart")}
            </button>
            <a href={whatsappHref}
              target="_blank" rel="noopener noreferrer"
              aria-label={l === "fr" ? "Commander via WhatsApp" : l === "ar" ? "طلب عبر واتساب" : "Order via WhatsApp"}
              className="flex-shrink-0 w-12 h-12 rounded-xl border border-gray-200 flex items-center justify-center transition-colors hover:bg-gray-50">
      ...
      {/* Spacer so the sticky Buy Box doesn't cover page content on mobile */}
      {product.in_stock && <div className="lg:hidden h-20" />}
    </div>
  );
}
